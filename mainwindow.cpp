...
// ui->VistaSuperiorQuickWidget->setResizeMode(QQuickWidget::SizeRootObjectToView);
    ...
    configureSpin(ui->pitchRecoveryThresholdSpin, 3.0, 0.0, 45.0, 0.1, 2);
    ui->pitchRecoveryThresholdSpin->setSuffix(" deg");
    configureSpin(ui->pitchRecoveryBrakeMsSpin, 80.0, 0.0, 1000.0, 10.0, 0);
    ui->pitchRecoveryBrakeMsSpin->setSuffix(" ms");
    configureSpin(ui->accelRunawayDeltaSpin, 120.0, 0.0, 5000.0, 10.0, 0);
    configureSpin(ui->accelRunawayAbsSpin, 450.0, 0.0, 5000.0, 10.0, 0);
    configureSpin(ui->accelRunawayTrendLimitSpin, 4.0, 1.0, 255.0, 1.0, 0);
    configureSpin(ui->accelRunawayAbsLimitSpin, 4.0, 1.0, 255.0, 1.0, 0);
    configureSpin(ui->accelAdaptiveStepSpin, 0.10, 0.0, 10.0, 0.05, 2);
    ui->accelAdaptiveStepSpin->setSuffix(" deg");
    configureSpin(ui->accelAdaptiveLimitSpin, 10.0, 0.0, 100.0, 0.1, 1);
    ...
    connect(ui->pitchRecoveryApplyButton, &QPushButton::clicked, this, [this]() {
        const float thresholdDeg = static_cast<float>(ui->pitchRecoveryThresholdSpin->value());
        const float brakeMs = static_cast<float>(ui->pitchRecoveryBrakeMsSpin->value());
        enviarFloat(CMD_PID_PITCH_LIM_INCLI, thresholdDeg);
        enviarFloat(CMD_PID_PITCH_CORECCION_RCSP, brakeMs);
        ui->TxTextEdit->append(QString("TX: PITCH RECOVERY TH=%1deg BRAKE=%2ms")
                                   .arg(thresholdDeg, 0, 'f', 2)
                                   .arg(brakeMs, 0, 'f', 0));
    });

    connect(ui->accelRunawayModeButton, &QPushButton::toggled, this, [this](bool enabled) {
        updateAccelRunawayModeButton(enabled);
        sendAccelRunawayConfig();
    });

    connect(ui->accelRunawayApplyButton, &QPushButton::clicked, this, [this]() {
        sendAccelRunawayConfig();
    });
    ...
    connect(ui->accelLogStartButton, &QPushButton::clicked, this, [this]() {
        if (!enviarComando(CMD_ACCEL_LOG_START, QByteArray())) {
            accelLogReceiving = false;
            ui->accelLogStatusLabel->setText("Start command not sent");
            return;
        }

        accelLogRows.clear();
        accelLogReceiving = true;
        accelLogExpectedSamples = 0;
        ui->accelLogTextEdit->clear();
        ui->accelLogStatusLabel->setText("Recording...");
        ui->TxTextEdit->append("TX: ACCEL LOG START");

        QTimer::singleShot(12000, this, [this]() {
            if (!accelLogReceiving) {
                return;
            }

            accelLogReceiving = false;
            updateAccelLogPreview();
            ui->accelLogStatusLabel->setText(QString("Timeout: %1/%2 samples")
                                                 .arg(accelLogRows.size())
                                                 .arg(accelLogExpectedSamples));
            ui->InfoTextEdit->append("Accel log receive timeout.");
        });
    });

    connect(ui->accelLogCopyButton, &QPushButton::clicked, this, [this]() {
        QGuiApplication::clipboard()->setText(buildAccelLogCsv());
        ui->InfoTextEdit->append("Accel log CSV copied.");
    });

    connect(ui->accelLogSaveButton, &QPushButton::clicked, this, [this]() {
...
QString MainWindow::buildAccelLogCsv() const
{
    QString csv;
    QTextStream out(&csv);
    out << "sample_index,time_s,accel_x_raw,pitch_angle_deg\n";

    for (const AccelLogRow &row : accelLogRows) {
        out << row.sampleIndex << ','
            << QString::number(row.sampleIndex / 100.0, 'f', 2) << ','
            << row.accelXRaw << ','
            << QString::number(row.pitchCdeg / 100.0, 'f', 2) << '\n';
    }
...
void MainWindow::handleUnerV1Packet(uint8_t cmd, uint8_t flags, uint8_t seq, const QByteArray &payload)
{
    ...
    case CMD_ACK:
        if (payload.size() >= 3) {
            ...
            if (ackCmd == CMD_ACCEL_LOG_START) {
                ui->InfoTextEdit->append(ackStatus == 0 ? "Accel log capture started." :
                                                          "Accel log rejected: capture busy.");
                if (ackStatus != 0) {
                    accelLogReceiving = false;
                    ui->accelLogStatusLabel->setText("Busy");
                }
            }
    ...
    case CMD_DATA:
    {
        ...
        updateAccelRunawayModeButton((datos.accelFlags & 0x01U) != 0);
    ...
    case CMD_ACCEL_LOG_CHUNK:
    {
        if (payload.size() < 7) {
            ui->RxTextEdit->append("ACCEL LOG chunk too short");
            break;
        }

        const uchar *p = reinterpret_cast<const uchar *>(payload.constData());
        const quint8 captureId = static_cast<quint8>(payload.at(0));
        const quint16 totalSamples = qFromLittleEndian<quint16>(p + 2);
        const quint16 firstIndex = qFromLittleEndian<quint16>(p + 4);
        const quint8 sampleCount = static_cast<quint8>(payload.at(6));
        const int expectedSize = 7 + (sampleCount * 6);

        if (payload.size() < expectedSize) {
            ui->RxTextEdit->append("ACCEL LOG chunk payload mismatch");
            break;
        }

        if (!accelLogReceiving || captureId != accelLogCaptureId) {
            accelLogRows.clear();
            accelLogReceiving = true;
            accelLogCaptureId = captureId;
            accelLogExpectedSamples = totalSamples;
            accelLogRows.resize(totalSamples);
            ui->accelLogStatusLabel->setText("Receiving...");
        } else if (accelLogRows.size() < totalSamples) {
            accelLogRows.resize(totalSamples);
            accelLogExpectedSamples = totalSamples;
        }

        for (quint8 i = 0; i < sampleCount; i++) {
            const int offset = 7 + (i * 6);
            const quint16 sampleIndex = qFromLittleEndian<quint16>(p + offset);
            if (sampleIndex >= static_cast<quint16>(accelLogRows.size())) {
                continue;
            }

            AccelLogRow row;
            row.sampleIndex = sampleIndex;
            row.accelXRaw = qFromLittleEndian<qint16>(p + offset + 2);
            row.pitchCdeg = qFromLittleEndian<qint16>(p + offset + 4);
            accelLogRows[sampleIndex] = row;
        }
        ...
        if (receivedCount >= totalSamples && totalSamples > 0) {
            ...
            ui->accelLogStatusLabel->setText(QString("Receiving %1/%2")
                                                 .arg(receivedCount)
                                                 .arg(totalSamples));
        }
        break;
    ...
    case CMD_ACCEL_LOG_DONE:
    {
        if (payload.size() >= 3) {
            const uchar *p = reinterpret_cast<const uchar *>(payload.constData());
            accelLogCaptureId = static_cast<quint8>(payload.at(0));
            accelLogExpectedSamples = qFromLittleEndian<quint16>(p + 1);
        }
...
void MainWindow::updateAccelRunawayModeButton(bool enabled)
{
    if (ui->accelRunawayModeButton->isChecked() != enabled) {
        ui->accelRunawayModeButton->blockSignals(true);
        ui->accelRunawayModeButton->setChecked(enabled);
        ui->accelRunawayModeButton->blockSignals(false);
    }
    ui->accelRunawayModeButton->setText(enabled ? "Detection ON" : "Detection OFF");
}

void MainWindow::sendAccelRunawayConfig()
{
    QByteArray payload;
    auto appendFloatLE = [&payload](float value) {
        quint32 raw;
    ...
    appendFloatLE(static_cast<float>(ui->accelRunawayDeltaSpin->value()));
    appendFloatLE(static_cast<float>(ui->accelRunawayAbsSpin->value()));
    appendFloatLE(static_cast<float>(ui->accelRunawayTrendLimitSpin->value()));
    appendFloatLE(static_cast<float>(ui->accelRunawayAbsLimitSpin->value()));
    appendFloatLE(static_cast<float>(ui->accelAdaptiveStepSpin->value()));
    appendFloatLE(static_cast<float>(ui->accelAdaptiveLimitSpin->value()));
    appendFloatLE(static_cast<float>(ui->accelAdaptiveResetAbsSpin->value()));
    appendFloatLE(static_cast<float>(ui->accelAdaptiveResetCountSpin->value()));
    payload.append(ui->accelRunawayModeButton->isChecked() ? char(1) : char(0));

    if (enviarComando(CMD_ACCEL_RUNAWAY_CONFIG, payload)) {
        ui->TxTextEdit->append(QString("TX: ACCEL RUNAWAY CONFIG %1")
                                   .arg(ui->accelRunawayModeButton->isChecked() ? "ON" : "OFF"));
    }
}
...
void MainWindow::on_accelAdaptiveLimitSpin_textChanged(const QString &arg1)
{
